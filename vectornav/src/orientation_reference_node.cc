...
private:
    void imuCallback(const vectornav_msgs::msg::ImuGroup::SharedPtr msg) {
        if (!reference_orientation_set_) {
            ...
            if (calibration_samples_collected_ < calibration_samples_) {
    ...
    void calculateReferenceOrientation() {
        ...
        reference_orientation_.x = rotation.x();
        reference_orientation_.y = rotation.y();
        reference_orientation_.z = rotation.z();
        reference_orientation_.w = rotation.w();
        
        reference_orientation_set_ = true;
        
        RCLCPP_INFO(get_logger(), "Reference orientation calculated:");
        RCLCPP_INFO(get_logger(), "  Gravity vector: [%.3f, %.3f, %.3f]", 
                   avg_gravity.x, avg_gravity.y, avg_gravity.z);
    ...
    void publishCompensatedImu(const vectornav_msgs::msg::ImuGroup::SharedPtr msg) {
        ...
        if (msg->group_fields & vectornav_msgs::msg::ImuGroup::IMUGROUP_ACCEL) {
            ...
            if (current_attitude_.w != 0.0) {  // Check if we have current attitude
                ...
                // Convert reference orientation to tf2 quaternion
                ...
                tf2::Quaternion rotation_from_ref = current_q * reference_q.inverse();
    ...
    tf2::Quaternion quaternionFromVectors(const tf2::Vector3& from, const tf2::Vector3& to) {
        // Calculate quaternion that rotates from vector 'from' to vector 'to'
